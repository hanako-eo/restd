...
impl<T: ?Sized> NonNull<T> {
    ...
    pub const unsafe fn copy_from(self, dest: NonNull<T>, count: usize)
    where
        T: Sized
    {
        self.0.copy_from(dest.as_ptr(), count)
    }

    pub unsafe fn copy_from_nonoverlapping(self, dest: NonNull<T>, count: usize)
    where
        T: Sized
    {
